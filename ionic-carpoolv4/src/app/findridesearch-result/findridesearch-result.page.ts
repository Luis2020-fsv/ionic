...
export class FindridesearchResultPage implements OnInit {
  ...
  starsCount: number;
  ridesList: any;
  constructor(public service: DataService, public route: Router, private crudService: CrudService) {
  ...
  ngOnInit() {
    this.crudService.read_Ride().subscribe(data=>{
      this.ridesList = data.map(e => {
        return {
          ...
          name: 'Cristina',
          car: 'Toyota prius 007,G5 7856, Mini white',
          numberPlate: e.payload.doc.data()['numberPlate'],
          cost: e.payload.doc.data()['cost']
        };
      })
      console.log(this.ridesList);
    });
  }
  goforBooking(cards) {
    console.log(cards);
